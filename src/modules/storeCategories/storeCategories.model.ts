...
const StoreCategories = mongoose.model<IStoreCategories>(
  "StoreCategories",
...
export const getStoreCategoryById = (id: string) =>
  StoreCategories.findById(id);

export const createStoreCategory = (storeCategoryData: IStoreCategories) =>
  new StoreCategories(storeCategoryData)
    .save()
    .then((CategoryData: IStoreCategories) =>
      CategoryData.toObject()
    );

export const updateStoreCategoryById = (
  id: string,
  storeCategoryData: Partial<IStoreCategories>
) => StoreCategories.findByIdAndUpdate(id, storeCategoryData, { new: true });

export const deleteStoreCategoryById = (id: string) =>
  StoreCategories.findByIdAndDelete(id);
